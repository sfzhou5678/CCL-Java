...
public class CCLearner_Feature {
    ...
    public static String config_file = "E:\\Dropbox\\代码搜索_编程辅助\\开源代码\\CCLearner-master\\CCLearner.conf";
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            String sql = "select A.type as type1, A.name as name1, A.startline as startline1, A.endline as endline1, "
                    ...
                    + "( "
                    + "select * from clones "
                    + "where functionality_id = 4 "     // TODO 只取4吗？
                    + sql_type[i]
                    + ") M "
            ...
            // TODO: 2018/4/8 在哪里控制的train的数据量？ 刚好训练集和测试集之和等于原数据量??
            while (res.next()) {
                ...
                // row[8]表示目标文件所在的funcType文件(2-11之间的一个值)
                // row[0]表示某type下的大文件夹(如selected, sampled)
                // row[1]表示文件名1，row[2]和row[3]表示第一个文件中函数1的起始行数和终止行数
                // row[4-7]与row[0-3]对应, 分别代表第二个方法的大文件夹、文件名、起始行数、终止行数
    ...
    public static void Generate_TrueClones() {
        ...
        // FIXME: 2018/4/8 这里是遍历计算行数位置来确定目标函数???? 这个做法有点奇葩
            ...
            for (int j = 0; j < methodVectorList2.size(); j++) {
                if (i != rec_i && j != rec_j) {
                    ...
                    if (feature_num == 8) {
                        ...
                        // FIXME: 2018/4/8 这种负采样方式有问题，容易导致数据偏靠前
